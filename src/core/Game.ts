...
export class Game {
  ...
  /**
   * Currently selected road segment id (popover open). Road segments are the
   * primary way to interact with road carriers — the player clicks the road
   * itself (not the moving worker) to delete the corridor or swap the worker
   * with a donkey. See `.claude/ROAD_SELECTION.md`.
   */
  public selectedRoadSegmentId: number | null = null;
  ...
  private setupEventListeners(): void {
    ...
    eventBus.on('road_segment:replace_with_donkey', (data: { segmentId: number }) => {
      if (!this.replaceRoadSegmentWorkerWithDonkey(data.segmentId)) {
        eventBus.emit('build:failed', {
          reason: 'No donkey available at headquarters',
        });
      }
    });
    eventBus.on('road_segment:return_donkey', (data: { segmentId: number }) => {
      if (!this.returnRoadSegmentDonkeyToHq(data.segmentId)) {
        eventBus.emit('build:failed', {
          reason: 'This carrier is not using a donkey',
        });
      }
    });
    eventBus.on('road_segment:delete', (data: { segmentId: number }) => {
      this.deleteRoadSegment(data.segmentId);
    });
    eventBus.on('road_segment:deselect', () => this.deselectRoadSegment());
  ...
  private scheduleSegmentRecalc(): void {
    ...
    this.segmentRecalcTimer = setTimeout(() => {
      ...
      // Segment ids are re-generated by `recalculate`; relink the popover to
      // the new id (or drop the selection if the corridor merged / vanished).
      this.refreshRoadSegmentSelection();
    }, 200);
  }
  ...
  private gameLoop = (currentTime: number): void => {
    ...
    // Track hovered entity for tooltips. In view mode, hovering a road corridor
    // (not a building) highlights the whole segment instead of any single
    // worker — clicks then select the road, see `selectEntityAt`.
    if (this.inputSystem.hoverGridPos) {
      const hx = this.inputSystem.hoverGridPos.x;
      const hy = this.inputSystem.hoverGridPos.y;
      const hoveredSegment = mode === 'view' ? this.findRoadSegmentAtTile(hx, hy) : null;
      const hoveredEntity =
        hoveredSegment != null
          ? null
      ...
      this.renderSystem.hoveredRoadSegmentTiles = hoveredSegment?.tiles ?? null;
      this.inputSystem.setHoverPointerActive(!!hoveredSegment);
    } else {
      this.renderSystem.hoveredEntityId = null;
      this.renderSystem.hoveredRoadSegmentTiles = null;
  ...
  /**
   * Returns the road segment whose *corridor* contains the given tile.
   * Wraps the pure helper from `roadSegmentSelection.ts` with a tile-state
   * guard (must be explored open road) so a click never lands on a fogged
   * cell or on the road tile under a building footprint.
   */
  private findRoadSegmentAtTile(gridX: number, gridY: number): RoadSegment | null {
    if (gridX < 0 || gridY < 0) return null;
    const tile = this.tileMap.getTile(gridX, gridY);
    if (!tile?.isExplored() || !tile.hasRoad || tile.isOccupied()) return null;
    return findRoadSegmentAtTile(roadSegmentManager.getSegments(), gridX, gridY);
  }

  /** Currently selected road segment (looked up live from the segment graph). */
  public getSelectedRoadSegment(): RoadSegment | null {
    if (this.selectedRoadSegmentId == null) return null;
    return roadSegmentManager.getSegments().find(s => s.id === this.selectedRoadSegmentId) ?? null;
  }

  /**
   * The selection key for a road segment is structural (its first + last tile)
   * because `RoadSegmentManager.recalculate` assigns brand new numeric ids on
   * every pass. Whenever the popover wants to find "the same segment again"
   * after a recalc it uses this fingerprint to relocate it.
   */
  private selectedRoadSegmentFingerprint: string | null = null;

  /** Public read-only access to a segment's tile path (for hover rendering). */
  public getRoadSegmentById(segmentId: number): RoadSegment | null {
    return roadSegmentManager.getSegments().find(s => s.id === segmentId) ?? null;
  }
  ...
  selectEntityAt(x: number, y: number, clientX?: number, clientY?: number): void {
    ...
    // Road segment selection (replaces the old "click the moving worker" flow).
    // Clicking the visible road corridor is far easier than chasing a sprite.
    const roadSegment = this.findRoadSegmentAtTile(x, y);
    if (roadSegment) {
      this.clearSurveyPending();
      this.selectedEntity = null;
      if (this.selectedRoadSegmentId === roadSegment.id) {
        this.deselectRoadSegment();
      } else {
    ...
    if (foundEntity) {
      ...
      // Clicked on empty space - deselect both buildings and road segments
      this.selectedEntity = null;
      this.deselectRoadSegment();
      this.updateSelectionUI();
    }
  ...
  private updateSelectionUI(): void {
    // Selecting a building cancels any road segment selection so the two
    // popovers can never be on screen at the same time.
    if (this.selectedEntity && this.selectedRoadSegmentId != null) {
      this.deselectRoadSegment();
    }
    if (this.selectedEntity) {
      const building = this.selectedEntity.getComponent(Building);
  ...
  /** Emit `road_segment:selected` and route the selection through the UI layer. */
  private selectRoadSegment(seg: RoadSegment): void {
    this.selectedRoadSegmentId = seg.id;
    this.selectedRoadSegmentFingerprint = fingerprintRoadSegment(seg);
    this.renderSystem.selectedRoadSegmentTiles = seg.tiles;
    this.updateSelectionUI();
    this.emitRoadSegmentSelectedPayload(seg);
  }

  /** Drops the segment selection and tells the UI layer to close the popover. */
  public deselectRoadSegment(): void {
    if (this.selectedRoadSegmentId == null) return;
    this.selectedRoadSegmentId = null;
    this.selectedRoadSegmentFingerprint = null;
    this.renderSystem.selectedRoadSegmentTiles = null;
    eventBus.emit('road_segment:deselected');
  }

  private emitRoadSegmentSelectedPayload(seg: RoadSegment): void {
    const workerEntity =
      seg.assignedWorkerId != null
        ? this.entities.find(e => e.id === seg.assignedWorkerId && e.active) ?? null
        : null;
    const worker = workerEntity?.getComponent(Worker) ?? null;
    const carrierType: 'worker' | 'donkey' | null = worker
      ? worker.carrierType === 'donkey'
        ? 'donkey'
        : 'worker'
      : null;
    eventBus.emit('road_segment:selected', {
      segmentId: seg.id,
      tiles: seg.tiles,
      assignedWorkerId: seg.assignedWorkerId,
      carrierType,
      donkeysAtHq: this.donkeysAtHq,
      canReplaceWithDonkey:
        seg.assignedWorkerId != null
          ? this.canReplaceRoadWorkerWithDonkey(seg.assignedWorkerId)
          : false,
      canReturnDonkeyToHq:
        seg.assignedWorkerId != null
          ? this.canReturnRoadDonkeyToHq(seg.assignedWorkerId)
          : false,
    });
  }

  /**
   * Re-emit a fresh payload for whichever road segment is currently selected.
   * Called after the segment graph mutates (recalc, worker replacement,
   * donkey return) so the popover reflects the new worker / counters.
   */
  public refreshRoadSegmentSelection(): void {
    if (this.selectedRoadSegmentId == null) return;
    const seg = this.getSelectedRoadSegment();
    if (!seg) {
      // Segment vanished (corridor was deleted or merged into a junction).
      // Try to relocate it by fingerprint before giving up.
      if (this.selectedRoadSegmentFingerprint) {
        const fp = this.selectedRoadSegmentFingerprint;
        const replacement = roadSegmentManager
          .getSegments()
          .find(s => fingerprintRoadSegment(s) === fp);
        if (replacement) {
          this.selectedRoadSegmentId = replacement.id;
          this.renderSystem.selectedRoadSegmentTiles = replacement.tiles;
          this.emitRoadSegmentSelectedPayload(replacement);
          return;
        }
      }
      this.deselectRoadSegment();
  ...
  /**
   * Delete every corridor tile of a road segment. Shared endpoints (junctions
   * and building entrances) are preserved so the rest of the network and the
   * building footprint aren't broken alongside the chosen road.
   */
  public deleteRoadSegment(segmentId: number): boolean {
    const seg = this.getRoadSegmentById(segmentId);
    if (!seg) return false;
    const tiles = getDeletableSegmentTiles(seg);
    if (tiles.length === 0) {
      this.deselectRoadSegment();
      return false;
    }
    ...
    for (const t of tiles) {
      const tile = this.tileMap.getTile(t.x, t.y);
      if (!tile || !tile.hasRoad || tile.isOccupied()) continue;
      tile.hasRoad = false;
      roadSegmentManager.removeRoad(t.x, t.y);
      removed++;
      this.renderSystem.spawnEraseSmoke(t.x, t.y);
    }
  ...
  /**
   * Swap the road carrier on a segment with a donkey (or back to a worker).
   * Wrappers around the existing entity-id-based public APIs so the road
   * popover only has to know about segment ids.
   */
  public replaceRoadSegmentWorkerWithDonkey(segmentId: number): boolean {
    const seg = this.getRoadSegmentById(segmentId);
    if (!seg?.assignedWorkerId) return false;
    const ok = this.replaceRoadWorkerWithDonkey(seg.assignedWorkerId);
    if (ok) this.refreshRoadSegmentSelection();
    return ok;
  }

  public returnRoadSegmentDonkeyToHq(segmentId: number): boolean {
    const seg = this.getRoadSegmentById(segmentId);
    if (!seg?.assignedWorkerId) return false;
    const ok = this.returnRoadDonkeyToHq(seg.assignedWorkerId);
    if (ok) this.refreshRoadSegmentSelection();
    return ok;
  }
