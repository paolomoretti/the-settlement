...
import {
  roadSegmentManager,
  RoadSegment,
} from "@/economics/RoadSegmentManager";
...
export class Game {
  ...
  constructor(
    ...
    this.movementSystem = new MovementSystem((entity) =>
      this.isEntitySimulationActive(entity),
    );
    ...
    this.workers = new GameWorkerRegistry({
      ...
      claimToolSpecialistForDispatch: (tool: string) =>
        this.claimToolSpecialistForDispatch(tool),
      returnToolSpecialistToHq: (tool: string) =>
        this.addToolSpecialistToHqPool(tool, 1),
      onMilitarySpecialistReturnedToHq: () =>
        this.addMilitarySpecialistToHqPool(1),
      onDonkeyReturnedToHq: () => this.onDonkeyReturnedToHq(),
      isEntitySimulationActive: (entity) =>
        this.isEntitySimulationActive(entity),
  ...
  private normalizeProductionPriorities(
    raw?: unknown,
  ): ProductionPriorityState {
  ...
  setBuildingPriority(buildingType: string, priority: number): void {
    this.buildingPriorities[buildingType] = Math.min(
      100,
  ...
  private sortEntitiesByBuildingPriority(entities: Entity[]): Entity[] {
    return entities
      ...
      .sort((a, b) => {
        const delta =
          this.getBuildingPriorityForEntity(b) -
          this.getBuildingPriorityForEntity(a);
  ...
  private setupEventListeners(): void {
    ...
    eventBus.on("erase:tile", (data: { x: number; y: number }) =>
      this.eraseAt(data.x, data.y),
    );
    ...
    eventBus.on("check:drag_selected", (data) =>
      this.checkDragSelected(data.x, data.y),
    );
  ...
  private updateEnemyRealmActivation(): void {
    ...
    for (const realm of this.enemyRealms) {
      ...
      if (!this.playerTerritoryTouchesRealm(realm, playerLayer.unionU))
        continue;
      realm.activated = true;
      this.seedEnemyRealmDecorations(realm);
      console.log(
        `${realm.factionId} activated as player territory reached the realm.`,
      );
    }
  }
  ...
  private playerTerritoryTouchesRealm(
    ...
    const margin = 1;
    for (
      let y = realm.bounds.y - margin;
      y < realm.bounds.y + realm.bounds.height + margin;
      y++
    ) {
      for (
        let x = realm.bounds.x - margin;
        x < realm.bounds.x + realm.bounds.width + margin;
        x++
      ) {
  ...
  private dispatchBredDonkeyToHq(sourceEntityId: number, count: number): void {
    const source = this.entities.find(
      (e) => e.id === sourceEntityId && e.active,
    );
    if (!source) {
    ...
    const startX =
      roadTile?.x ??
      (pos && building ? pos.x + Math.floor(building.width / 2) : null);
    const startY =
      roadTile?.y ??
      (pos && building ? pos.y + Math.floor(building.height / 2) : null);
  ...
  private hasAnyUnroutedProductionOutput(): boolean {
    for (const entity of this.sortEntitiesByBuildingPriority(this.entities)) {
      ...
      if (!production || !building?.isComplete() || !building.isActive)
        continue;
  ...
  private rescueStrandedItems(): void {
    ...
    for (const [key, items] of [
      ...transportManager.getJunctionItemsMap().entries(),
    ]) {
    ...
    for (const { x, y, resourceType, destinationEntityId } of collected) {
      ...
      for (const seg of segsHere) {
        if (
          transportManager.getDirectionIndex(seg.id, destinationEntityId) !==
          undefined
        ) {
          hasRoute = true;
          break;
      ...
      if (hasRoute) {
        ...
        for (const seg of segsHere) {
          if (
            transportManager.getDirectionIndex(seg.id, baseCampId) !== undefined
          ) {
  ...
  private cancelInvalidTransportTasks(): void {
    for (const segment of roadSegmentManager.getSegments()) {
      ...
      const entity = this.entities.find(
        (e) => e.id === segment.assignedWorkerId && e.active,
      );
      if (!entity) continue;
  ...
  private findBuildingEntityAt(x: number, y: number): Entity | null {
    const found = this.entities.find((entity) => {
      ...
      return (
        x >= pos.x &&
        x < pos.x + building.width &&
        y >= pos.y &&
        y < pos.y + building.height
      );
    });
    return found ?? null;
  }
      ...
      if (garrisonWorkerIds.length > 0) {
        if (isPlayerOwned(entity)) {
          ...
          for (const workerId of garrisonWorkerIds) {
            const workerEntity = this.entities.find(
              (e) => e.id === workerId && e.active,
            );
  ...
  private hasBuildingConnectedRoad(
    ...
    const hasConnectedRoadAt = (x: number, y: number): boolean =>
      connectedRoads.has(`${x},${y}`);
    const entrance = building.getEntranceOffset();
    if (entrance) {
  ...
  private cleanupDemolitionSites(): void {
    const expiresBefore =
      ...
      DEMOLITION_FIRE_DURATION_MS -
      DEMOLITION_SCORCH_DURATION_MS;
    const before = this.demolitionSites.length;
    this.demolitionSites = this.demolitionSites.filter(
      (site) => site.startedAt > expiresBefore,
    );
    if (this.demolitionSites.length !== before)
      this.syncDemolitionSitesToRender();
  }
  ...
  private buildBaseCamp(x: number, y: number): void {
    ...
    console.log(
      `Base Camp (${building.width}x${building.height}) established at (${x}, ${y})`,
    );
  ...
  private seedEnemyVillage(plan: EnemyVillagePlan): void {
    ...
    this.updateBuildingRoadConnections();
    console.log(
      `${plan.factionId} village generated at (${plan.bounds.x}, ${plan.bounds.y})`,
    );
  }
    ...
    if (plan.seedGarrison) {
      ...
      if (typeof cap === "number" && cap > 0) {
        ...
        const fillRatio = Math.max(
          0.1,
          Math.min(1, plan.garrisonFillRatio ?? 0.5),
        );
        const fillCount = Math.min(
          cap,
          Math.max(1, Math.ceil(cap * fillRatio)),
        );
        const rank = plan.garrisonRank ?? 1;
        for (let i = 0; i < fillCount; i++) {
          const soldier = createMilitaryWorker(
            plan.x + 0.5,
            plan.y + 0.5,
            rank,
          );
  ...
  private seedEnemyStreetWorkers(plan: EnemyVillagePlan): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const end =
        roadCells[
          Math.floor(((i + 0.5) / count) * roadCells.length) % roadCells.length
        ]!;
  ...
  private seedEnemyRealmDecorations(realm: EnemyVillagePlan): void {
    for (const entity of this.entities) {
      if (!entity.active || getEntityFaction(entity) !== realm.factionId)
        continue;
      const building = entity.getComponent(Building);
  ...
  private buildGeneric(buildingType: BuildingType, x: number, y: number): void {
    ...
    if (building.buildTimeSec > 0) {
      if (hasCosts) {
        building.startAwaitingMaterials(
          building.buildTimeSec,
          buildingDef.buildCost,
        );
  ...
  private getToolSpecialistsAtHqCount(): number {
    return Object.values(this.toolSpecialistsAtHq).reduce(
      (sum, n) => sum + n,
      0,
    );
  }
  ...
  private getToolSpecialistsInTransitCount(): number {
    return Object.values(this.workers.getToolDispatchCounts()).reduce(
      (sum, n) => sum + n,
      0,
    );
  }
  ...
  private addToolSpecialistToHqPool(tool: string, count: number = 1): void {
    if (count <= 0) return;
    this.toolSpecialistsAtHq[tool] =
      (this.toolSpecialistsAtHq[tool] || 0) + count;
  }
  ...
  private getRoadSegmentForWorkerId(workerId: number): RoadSegment | null {
    return (
      roadSegmentManager
        .getSegments()
        .find((seg) => seg.assignedWorkerId === workerId) ?? null
    );
  }

  public canReplaceRoadWorkerWithDonkey(workerEntityId: number): boolean {
    const entity = this.entities.find(
      (e) => e.id === workerEntityId && e.active,
    );
    const worker = entity?.getComponent(Worker);
    if (!worker || !this.getRoadSegmentForWorkerId(workerEntityId))
      return false;
  ...
  public replaceRoadWorkerWithDonkey(workerEntityId: number): boolean {
    ...
    if (this.donkeysAtHq > 0) {
      this.donkeysAtHq--;
      this.cancelRoadWorkerTransportForReassignment(entity);
      const replacement = this.dispatchRoadCarrierFromHq(
        targetSegment,
        "donkey",
      );
      if (!replacement) {
      ...
      const donorSegment = roadSegmentManager.getSegments().find((seg) => {
        if (
          seg.assignedWorkerId === workerEntityId ||
          seg.assignedWorkerId == null
        )
          return false;
        const donorEntity = this.entities.find(
          (e) => e.id === seg.assignedWorkerId && e.active,
        );
      ...
      const donor =
        donorSegment?.assignedWorkerId != null
          ? this.entities.find(
              (e) => e.id === donorSegment.assignedWorkerId && e.active,
            )
          : null;
      ...
      const donorReplacement = this.dispatchRoadCarrierFromHq(
        donorSegment,
        "worker",
      );
  ...
  public getSpecializedWorkersSummary(): Array<{
    ...
    for (const e of this.entities) {
      ...
      toolAssigned[b.assignedToolSpecialist] =
        (toolAssigned[b.assignedToolSpecialist] || 0) + 1;
    }
    ...
    for (const tool of toolIds) {
      ...
      const resName =
        dataManager.getResource(tool as ResourceType)?.name || tool;
  ...
  public trainMilitary(
    ...
    const toastFail = (msg: string) => {
      ...
      if (hp) {
        ...
        const canvas = this.renderSystem["canvas"] as
          | HTMLCanvasElement
          | undefined;
    ...
    if (this.hasMilitaryFromBuildingInActiveAttack(buildingEntity.id))
      return false;
    ...
    const pending = this.workers.countMilitaryDispatchToBuilding(
      buildingEntity.id,
    );
    const emptySlots = building.militaryGarrison!.filter(
      (s) => s == null,
    ).length;
    ...
    if (!usePooledMilitary) {
      ...
      if (
        !hqStorage ||
        hqStorage.getAmount("sword") < 1 ||
        hqStorage.getAmount("shield") < 1
      ) {
    ...
    if (path.length === 0) {
      toastFail(
        "No road path from HQ to this fort — connect both to the same road network",
      );
      return false;
    }
    ...
    if (!silent) {
      ...
      if (hp) {
        ...
        const canvasEl = (
          this.renderSystem as unknown as { canvas: HTMLCanvasElement }
        ).canvas;
        this.renderSystem.showToast(
          "Soldier marching from HQ",
          canvasEl.clientWidth * 0.5,
          96,
        );
      }
    }
  ...
  private hasMilitaryFromBuildingInActiveAttack(
    buildingEntityId: number,
  ): boolean {
    return this.activeEnemyAttacks.some(
      (attack) =>
        attack.phase !== "complete" &&
        (attack.attackers.some(
          (p) => p.sourceBuildingId === buildingEntityId,
        ) ||
          attack.defenders.some(
            (p) => p.sourceBuildingId === buildingEntityId,
          ) ||
          attack.returnAssignments.some(
            (p) => p.buildingEntityId === buildingEntityId,
          )),
    );
  }
  ...
  private recheckConstructionMaterials(force = false): void {
    ...
    for (const entity of this.sortEntitiesByBuildingPriority(this.entities)) {
      ...
      for (const [res, needed] of Object.entries(
        building.constructionMaterials,
      )) {
        const delivered = building.materialsDelivered[res] || 0;
        ...
        for (const [, items] of transportManager.getJunctionItemsMap()) {
          for (const item of items) {
            if (
              item.destinationEntityId === entity.id &&
              item.resourceType === res
            ) {
              inTransit++;
            }
          }
        ...
        for (const seg of roadSegmentManager.getSegments()) {
          ...
          if (worker.carryingItems.length > 0) {
            ...
            inTransit += Math.max(
              1,
              worker.carryingAmount || worker.transportTask.amount || 1,
            );
          }
        }
    ...
    for (const seg of roadSegmentManager.getSegments()) {
      ...
      if (task.phase === "to_dropoff") {
        inTransit += worker?.carryingItems.length
          ? worker.carryingItems.filter(
              (item) => item.destinationEntityId === destEntityId,
            ).length
          : Math.max(1, worker?.carryingAmount || task.amount || 1);
      } else if (task.phase === "to_pickup" && task.sourceEntityId != null) {
  ...
  private countInTransitToBuildingForResource(
    destEntityId: number,
    resourceType: string,
  ): number {
    let n = 0;
    ...
    for (const [, items] of transportManager.getJunctionItemsMap()) {
      for (const item of items) {
        if (
          item.destinationEntityId === destEntityId &&
  ...
  private migrateLegacyProductionOutputsFromBuffer(entity: Entity): void {
    ...
    const spawnTile = this.baseCampEntity
      ? this.workers.getBaseCampSpawnTile()
      : null;
  ...
  private canAddProductionInputToLocalStorage(
    ...
    const freeAfterAccept = incomingAlreadyInTransit
      ? freeEff
      : freeEff - amount;

    const pipeline = (t: string) =>
      storage.getAmount(t) +
      this.countInTransitToBuildingForResource(entity.id, t);
  ...
  private rescueStuckMultiInputProductionStorage(entity: Entity): void {
    ...
    const maxShortfall = () =>
      Math.max(
        0,
        ...types.map((t) =>
          Math.max(0, (inputs[t] ?? 0) - storage.getAmount(t)),
        ),
      );
  ...
  private tryDispatchHqProductionInputsForBuilding(entity: Entity): void {
    ...
    if (fixedInputTypes.length > 0 && totalFixedInputAmount > 0) {
      for (const res of fixedInputTypes) {
        const quota = Math.max(
          ...
          Math.floor(
            storage.capacity *
              ((production.inputs[res] ?? 0) / totalFixedInputAmount),
          ),
        );
    ...
    const targetStoredOrInTransit = (res: string): number => {
      ...
      if (needFixed > 0) {
        return fixedInputTypes.length <= 1
          ? storage.capacity
          : (baseFixedQuota[res] ?? needFixed);
      }
    ...
    const stillNeedsInput = (res: string): boolean => {
      ...
      const pipeline =
        storage.getAmount(res) +
        this.countInTransitToBuildingForResource(entity.id, res);
      if (pipeline >= target) return false;
      for (const g of production.inputsAny) {
        if (!g.resourceTypes.includes(res as ResourceType)) continue;
        const groupPipeline = production.pipelineSumForInputsAnyGroup(
          g,
          storage,
          (r) => this.countInTransitToBuildingForResource(entity.id, r),
        );
    ...
    let dispatchSlots = Math.max(
      ...
      storage.capacity -
        ...
        this.countInTransitToBuilding(entity.id),
    );
  ...
  private tryConsumeMilitaryGoldAfterDelivery(
    destEntity: Entity,
    ...
    if (
      !storage ||
      !destBuilding.militaryGarrison ||
      !destBuilding.militaryWantsMoreGold()
    )
      return;
    ...
    if (slot) {
      const wEnt = this.entities.find(
        (e) => e.id === slot.workerEntityId && e.active,
      );
  ...
  private tryDispatchMilitaryGoldPayroll(): void {
    ...
    for (const entity of this.entities) {
      ...
      if (!building.militaryGarrison || !building.militaryWantsMoreGold())
        continue;
      if (!storage.canAccept("gold_coin")) continue;

      const inFlight = this.countInTransitToBuildingForResource(
        entity.id,
        "gold_coin",
      );
  ...
  private updateTransport(): void {
    ...
    for (const seg of segments) {
      ...
      const e = this.entities.find(
        (ent) => ent.id === seg.assignedWorkerId && ent.active,
      );
      if (!e) continue;
      const w = e.getComponent(Worker);
      if (
        w?.transportTask?.sourceEntityId != null &&
        w.transportTask.phase === "to_pickup"
      ) {
  ...
  private findDemandingBuilding(resourceType: string): Entity | null {
    for (const entity of this.entities) {
      ...
      if (
        !production
          .getAllInputResourceTypes()
          .includes(resourceType as ResourceType)
      )
        continue;
  ...
  private checkBuildingForOutput(
    buildingEntityId: number,
  ): { resourceType: string } | null {
    const bldgEntity = this.entities.find(
      (e) => e.id === buildingEntityId && e.active,
    );
  ...
  private tryStartTransport(
    ...
    for (let pickupIdx = 0; pickupIdx < 2; pickupIdx++) {
      ...
      if (
        ep.entityId != null &&
        !this.pendingBuildingPickups.has(ep.entityId)
      ) {
        const output = this.checkBuildingForOutput(ep.entityId);
        if (output) {
          let destEntityId: number | null = this.baseCampEntity?.id ?? null;
          const demandBuilding = this.findDemandingBuilding(
            output.resourceType,
          );
          if (demandBuilding) {
            const dirIdx = transportManager.getDirectionIndex(
              segment.id,
              demandBuilding.id,
            );
          ...
          let dirCheck = transportManager.getDirectionIndex(
            segment.id,
            destEntityId,
          );
          if (dirCheck === undefined) {
            this.kickTransportRoutesForNewOutput();
            dirCheck = transportManager.getDirectionIndex(
              segment.id,
              destEntityId,
            );
          }
    ...
    switch (task.phase) {
      case "to_pickup": {
        ...
        if (task.sourceEntityId != null) {
          const bldgEntity = this.entities.find(
            (e) => e.id === task.sourceEntityId && e.active,
          );
          if (bldgEntity) {
            const production = bldgEntity.getComponent(Production);
            ...
            if (
              production &&
              (takenAmount = production.removeFromBuffer(
                task.resourceType,
                requestedAmount,
              )) > 0
            ) {
              ...
              (takenAmount = bldgStorage.removeItem(
                task.resourceType,
                requestedAmount,
              )) > 0
            ) {
          ...
          if (
            pickupEpIdx >= 0 &&
            carriedItems.length < worker.transportCarryCapacity
          ) {
        ...
        const path = this.getSegmentPathToTile(
          segment,
          pos,
          task.dropoffPos.x,
          task.dropoffPos.y,
      ...
      case "to_dropoff": {
        ...
        worker.carryingItems = [];
        this.deliverTransportLoadItems(
          carriedItems,
          dropoffEp,
          task.dropoffPos,
        );
        ...
        if (
          this.tryChainedJunctionPickupAfterDropoff(
            segment,
            worker,
            movable,
            pos,
  ...
  private deliverTransportLoadItems(
    ...
    for (const item of items) {
      if (
        dropoffEp?.entityId === item.destinationEntityId &&
        item.destinationEntityId != null
      ) {
    ...
    for (const [destEntityId, destItems] of groupedFinalDeliveries) {
      if (destEntityId === this.baseCampEntity?.id) {
        const storage = this.baseCampEntity.getComponent(Storage);
        if (storage) {
          for (const [res, count] of this.groupTransportItemsByResource(
            destItems,
          )) {
      ...
      const destEntity = this.entities.find(
        (e) => e.id === destEntityId && e.active,
      );
      if (!destEntity) {
        ...
        for (const [res, count] of this.groupTransportItemsByResource(
          ...
          if (added > 0 && res === "gold_coin" && destBuilding) {
            for (let i = 0; i < added; i++) {
              this.tryConsumeMilitaryGoldAfterDelivery(
                destEntity,
                destBuilding,
              );
            }
  ...
  private groupTransportItemsByResource(
    items: TransportLoadItem[],
  ): Map<string, number> {
  ...
  private gameLoop = (currentTime: number): void => {
        ...
        if (this.inputSystem.getShiftKeyHeld()) {
          ...
          if (pathAnchor && (pathAnchor.x !== gx || pathAnchor.y !== gy)) {
            ...
            if (dragAnchor && (dragAnchor.x !== gx || dragAnchor.y !== gy)) {
              roadLineTiles = axisAlignedGridLine(
                dragAnchor.x,
                dragAnchor.y,
                gx,
                gy,
    ...
    this.renderSystem.setSurveyWorkerIdsOnTop(
      this.surveys.getActiveSurveyorWorkerIds(),
    );
    this.renderSystem.setSurveyOverlay(
      this.surveys.getOverlayForRender(this.simulationNowMs),
    );
      ...
      if (building && building.state === "under_construction") {
        ...
        if (building.isComplete()) {
          ...
          if (production && production.hasInputs()) {
            transportManager.computeRoutesToBuilding(
              roadSegmentManager.getSegments(),
              entity.id,
            );
          }
          ...
          if (
            st &&
            !st.isProductionStorage &&
            st.accepts?.includes("gold_coin")
          ) {
            transportManager.computeRoutesToBuilding(
              roadSegmentManager.getSegments(),
              entity.id,
          ...
          this.tryFinalizeWellAquifer(entity);
          if (
            typeof buildingDef?.military?.territoryVisionRadius === "number"
          ) {
            this.markTerritoryDirty();
          }
        }
  ...
  private updateDebugInfo(): void {
    ...
    if (entityCountElement)
      entityCountElement.textContent = this.entities.length.toString();
  }
  ...
  public isEnemyAttackTarget(entity: Entity): boolean {
    ...
    return (
      building.buildingType === "base_camp" || this.isMilitaryBuilding(building)
    );
  }
  ...
  public getEnemyAttackOptions(targetEntity: Entity): EnemyAttackOptions {
    ...
    for (const entity of this.entities) {
      ...
      const building = entity.getComponent(Building);
      if (!building?.militaryGarrison || !this.isMilitaryBuilding(building))
        continue;
    ...
    const totalAvailable =
      availableByRank[1] + availableByRank[2] + availableByRank[3];
    return {
      availableByRank,
  ...
  public startEnemyAttack(
    ...
    if (
      this.activeEnemyAttacks.some(
        (attack) =>
          attack.targetEntityId === targetEntity.id &&
          attack.phase !== "complete",
      )
    ) {
    ...
    const attackers = this.claimAttackersForEnemyAttack(
      targetCenter,
      requested,
    );
  ...
  private claimAttackersForEnemyAttack(
    ...
    const sourceBuildings = this.entities
      ...
      .sort((a, b) => {
        ...
        const ad = Math.max(
          Math.abs(ac.x - targetCenter.x),
          Math.abs(ac.y - targetCenter.y),
        );
        const bd = Math.max(
          Math.abs(bc.x - targetCenter.x),
          Math.abs(bc.y - targetCenter.y),
        );
        return ad - bd;
      });
  ...
  private claimDefendersForEnemyAttack(
    targetEntity: Entity,
  ): AttackParticipant[] {
  ...
  private getAttackStagingTiles(
    targetEntity: Entity,
  ): ActiveEnemyAttack["staging"] {
    const targetCenter = this.getBuildingCenter(targetEntity) ?? { x: 0, y: 0 };
    const attacker = this.findNearestOpenTile(
      targetCenter.x - 3,
      targetCenter.y,
      targetCenter,
    ) ??
    ...
    const defender = this.findNearestOpenTile(
      targetCenter.x - 1,
      targetCenter.y,
      targetCenter,
    ) ??
  ...
  private offsetAttackStaging(
    ...
    return (
      this.findNearestOpenTile(base.x + offset.x, base.y + offset.y, base) ??
      base
    );
  }
    ...
    for (let r = 0; r <= 5; r++) {
      for (let dy = -r; dy <= r; dy++) {
        for (let dx = -r; dx <= r; dx++) {
          ...
          if (avoid && gx === Math.floor(avoid.x) && gy === Math.floor(avoid.y))
            continue;
    ...
    const moveTo = (
      ...
      if (worker?.concealedInBuildingId != null) {
        const exit = this.getBuildingExitTile(
          this.entities.find((e) => e.id === p.sourceBuildingId) ??
            workerEntity,
          index,
        );
        if (exit) {
  ...
  private updateEnemyBorderPressure(): void {
    ...
    for (const realm of this.enemyRealms) {
      ...
      realm.nextAttackAt =
        ...
        (attempted
          ? cooldown
          : Math.min(cooldown, attackConfig.checkIntervalMs * 6));
    }
  }

  private getRealmAggressiveness(realm: EnemyVillagePlan): number {
    const override =
      dataManager.getGameConfig().enemyRealms.debugAggressivenessOverride;
    ...
    const index = this.enemyRealms.findIndex(
      (r) => r.factionId === realm.factionId,
    );
    const configured =
      dataManager.getGameConfig().enemyRealms.aggressivenessByVillageIndex[
        index
      ] ?? 0;
    realm.aggressivenessLevel = Math.max(
      0,
  ...
  private getAggressionTuningValue(
    ...
    return (
      ...
      values[Math.max(0, Math.min(5, aggressiveness))] ??
      fallback
    );
  }
  ...
  private hasActiveAttackForFaction(factionId: FactionId): boolean {
    return this.activeEnemyAttacks.some(
      (attack) =>
        attack.phase !== "complete" &&
        (attack.attackerFactionId === factionId ||
          attack.targetFactionId === factionId),
    );
  }
  ...
  private startEnemyRealmAttack(realm: EnemyVillagePlan): boolean {
    ...
    const attackers = this.claimFactionAttackers(
      realm.factionId,
      targetCenter,
      maxAttackers,
    );
    ...
    const candidates = this.entities
      .filter((entity) => {
        ...
        if (
          this.activeEnemyAttacks.some(
            (attack) =>
              attack.targetEntityId === entity.id &&
              attack.phase !== "complete",
          )
        )
          return false;
      ...
      .map((entity) => {
        ...
        const nearest = Math.min(
          ...enemySources.map((source) =>
            Math.max(
              Math.abs(source.x - center.x),
              Math.abs(source.y - center.y),
            ),
          ),
        );
      ...
      .filter(
        (
          entry,
        ): entry is { entity: Entity; distance: number; defenders: number } =>
          entry !== null,
      )
  ...
  private getFactionMilitaryCenters(
    ...
    return this.entities
      .filter((entity) => {
        if (!entity.active || getEntityFaction(entity) !== factionId)
          return false;
      ...
      .filter(
        (entry): entry is { entity: Entity; x: number; y: number } =>
          entry !== null,
      );
  }
  ...
  private claimFactionAttackers(
    ...
    const sources = this.getFactionMilitaryCenters(factionId)
      .filter(
        (source) =>
          Math.max(
            Math.abs(source.x - targetCenter.x),
            Math.abs(source.y - targetCenter.y),
          ) <= this.getEnemyAttackRangeCells(),
      )
      .sort((a, b) => {
        const ad = Math.max(
          Math.abs(a.x - targetCenter.x),
          Math.abs(a.y - targetCenter.y),
        );
        const bd = Math.max(
          Math.abs(b.x - targetCenter.x),
          Math.abs(b.y - targetCenter.y),
        );
        return ad - bd;
      });
  ...
  private reinforceEnemyRealmGarrison(
    realm: EnemyVillagePlan,
    now: number,
  ): void {
    const attackConfig = dataManager.getGameConfig().enemyRealms.attacks;
    ...
    const rank = this.getEnemyReinforcementRank(
      this.getRealmAggressiveness(realm),
    );
    const posts = this.entities
      ...
      .sort((a, b) => {
        const af =
          a.getComponent(Building)?.getMilitaryGarrisonFilledCount() ?? 0;
        const bf =
          b.getComponent(Building)?.getMilitaryGarrisonFilledCount() ?? 0;
        return af - bf;
      });
    ...
    for (const entity of posts) {
      ...
      const cap =
        dataManager.getBuilding(building.buildingType)?.military
          ?.soldierCapacity ?? 0;
      building.initMilitaryGarrison(cap);
  ...
  private updateEnemyRoadMaintenance(): void {
    ...
    let forwardBarracksRemaining = Math.max(
      0,
      config.maxForwardBarracksPerTick,
    );
    if (repairsRemaining <= 0 && forwardBarracksRemaining <= 0) return;

    for (const realm of this.enemyRealms) {
      if (realm.activated !== true || !this.isEnemyRealmVisible(realm))
        continue;
      if (
  ...
  private isEnemyRealmVisible(realm: EnemyVillagePlan): boolean {
    for (
      let y = realm.bounds.y;
      y < realm.bounds.y + realm.bounds.height;
      y++
    ) {
      for (
        let x = realm.bounds.x;
        x < realm.bounds.x + realm.bounds.width;
        x++
      ) {
  ...
  private repairOneEnemyRoadConnection(
    realm: EnemyVillagePlan,
    maxPathCells: number,
  ): boolean {
    const hq = this.findEnemyHeadquarters(realm.factionId);
    if (!hq) return false;
    const connectedRoads = this.getFactionHeadquartersConnectedRoads(
      realm.factionId,
      hq,
    );
    const disconnected = this.entities
      .filter((entity) => {
        ...
        if (this.hasBuildingConnectedRoad(pos, building, connectedRoads))
          return false;
        return (
          this.getEnemyRoadAnchorForBuilding(entity, realm.factionId) !== null
        );
      })
      .sort((a, b) => {
        const ac = this.getBuildingCenter(a);
        const bc = this.getBuildingCenter(b);
        const ah = this.getBuildingCenter(
          this.findEnemyHeadquarters(realm.factionId) ?? a,
        );
    ...
    for (const entity of disconnected) {
      const target = this.getEnemyRoadAnchorForBuilding(
        entity,
        realm.factionId,
      );
      if (!target) continue;
  ...
  private getFactionHeadquartersConnectedRoads(
    ...
    const isNetworkRoad = (x: number, y: number): boolean => {
      ...
      if (!tile.isOccupied())
        return this.territory.isInteriorCell(x, y, factionId);
      const occupant = this.entities.find(
        (entity) => entity.id === tile.occupiedBy,
      );
  ...
  private buildForwardBarracksForIsolatedMilitary(
    ...
    const isolatedPost = this.findIsolatedEnemyMilitaryPost(
      realm.factionId,
      hq,
      connectedRoads,
    );
    ...
    const targetAnchor = this.getEnemyRoadAnchorForBuilding(
      isolatedPost,
      realm.factionId,
    );
    const barracksSite = targetAnchor
      ? this.findForwardBarracksSite(
          realm.factionId,
          isolatedPost,
          targetAnchor,
          searchRadius,
    ...
    const barracksPlan: EnemyVillageBuildingPlan = {
      ...
      seedGarrison: true,
      garrisonFillRatio: 1,
      garrisonRank: this.getEnemyReinforcementRank(
        this.getRealmAggressiveness(realm),
      ),
    };
    const barracks = this.placeEnemyBuilding(
      barracksPlan,
      realm.factionId,
      false,
    );
    ...
    const barracksAnchor = this.getEnemyRoadAnchorForBuilding(
      barracks,
      realm.factionId,
    );
    const source = barracksAnchor
      ? this.findNearestEnemyRoadCell(
          realm.factionId,
          barracksAnchor,
          realm.bounds,
          connectedRoads,
    ...
    if (source && barracksAnchor) {
      const path = this.planEnemyRoadPath(
        source,
        barracksAnchor,
        realm.factionId,
      );
    ...
    const refreshedConnectedRoads = this.getFactionHeadquartersConnectedRoads(
      realm.factionId,
      hq,
    );
    ...
    const candidates = this.entities
      .filter((entity) => {
        if (
          !entity.active ||
          entity === headquarters ||
          getEntityFaction(entity) !== factionId
        )
          return false;
        const building = entity.getComponent(Building);
        const pos = entity.getComponent(Position);
  ...
  private findForwardBarracksSite(
    ...
    for (
      ...
      for (
        ...
        if (!this.canPlaceEnemyBuildingAt(x, y, width, height, factionId))
          continue;
        const footprintCenter = { x: x + width / 2, y: y + height / 2 };
        const distanceToPost =
          Math.abs(footprintCenter.x - center.x) +
          Math.abs(footprintCenter.y - center.y);
        const distanceToAnchor =
          Math.abs(footprintCenter.x - targetAnchor.x) +
          Math.abs(footprintCenter.y - targetAnchor.y);
        const frontierScore = this.distanceFromFactionFrontier(
          x,
          y,
          width,
          height,
        ...
        const score =
          distanceToPost + distanceToAnchor * 0.5 + frontierScore * 0.35;
  ...
  private hasEnemyBarracksNear(
    ...
    return this.entities.some((entity) => {
      ...
      if (!building || building.buildingType !== "barracks" || !otherCenter)
        return false;
      const distance =
        Math.abs(otherCenter.x - center.x) + Math.abs(otherCenter.y - center.y);
  ...
  private canPlaceEnemyBuildingAt(
    ...
    for (let dy = 0; dy < height; dy++) {
      for (let dx = 0; dx < width; dx++) {
        if (!this.territory.isInteriorCell(x + dx, y + dy, factionId))
          return false;
      }
    }
    return true;
  ...
  private distanceFromFactionFrontier(
    ...
    for (
      ...
      for (
        ...
        if (!this.territory.getLayer(factionId).frontier.has(`${cx},${cy}`))
          continue;
        const distance =
          Math.abs(cx - (x + width / 2)) + Math.abs(cy - (y + height / 2));
  ...
  private getEnemyRoadAnchorForBuilding(
    ...
    return (
      candidates.find((cell) =>
        this.canUseEnemyRoadPathCell(cell.x, cell.y, factionId),
      ) ?? null
    );
  }
  ...
  private canUseEnemyRoadPathCell(
    x: number,
    y: number,
    factionId: FactionId,
  ): boolean {
  ...
  private updateEnemyAttacks(): void {
    ...
    for (const attack of this.activeEnemyAttacks) {
      ...
      if (attack.phase === "marching") {
        const arrived = [...attack.attackers, ...attack.defenders].every(
          (p) => {
        ...
        if (attack.fallenWorkerId != null) {
          ...
          const fallen = this.entities.find(
            (e) => e.id === attack.fallenWorkerId,
          );
    ...
    this.activeEnemyAttacks = this.activeEnemyAttacks.filter(
      (attack) => attack.phase !== "complete",
    );
  ...
  private updateBattleAudio(now: number): void {
    const hasVisibleDuel = this.activeEnemyAttacks.some(
      (attack) =>
        attack.phase === "duel" && this.isAttackVisibleToPlayer(attack),
    );
    if (!hasVisibleDuel) {
      this.nextBattleClashAt = 0;
      return;
    ...
    audioManager.playSound(
      Math.random() < 0.5 ? "battle_clash_1" : "battle_clash_2",
    );
  ...
  private playBattleOutcomeSoundIfVisible(
  ...
  private setDuelistsCloseTogether(attack: ActiveEnemyAttack): void {
    if (attack.currentAttackerId == null || attack.currentDefenderId == null)
      return;
    const attacker = this.entities.find(
      (e) => e.id === attack.currentAttackerId,
    );
    const defender = this.entities.find(
      (e) => e.id === attack.currentDefenderId,
    );
  ...
  private resolveEnemyAttackDuel(attack: ActiveEnemyAttack, now: number): void {
    ...
    const attackerRoll =
      Math.random() * this.getMilitaryRankWeight(attackerRank);
    const defenderRoll =
      Math.random() * this.getMilitaryRankWeight(defenderRank);
    ...
    const winnerWorker = this.entities
      .find((e) => e.id === winnerId)
      ?.getComponent(Worker);
    if (winnerWorker) {
      winnerWorker.visualActivity = "combat_duel";
      winnerWorker.setState("working");
    }
    const loserWorker = this.entities
      .find((e) => e.id === loserId)
      ?.getComponent(Worker);
    if (loserWorker) {
  ...
  private finishEnemyAttackLoss(attack: ActiveEnemyAttack): void {
    ...
    const survivorIds = [
      attack.currentDefenderId,
      ...attack.defenderQueue,
  ...
  private finishEnemyAttackWin(attack: ActiveEnemyAttack): void {
    ...
    const survivingAttackers = [
      attack.currentAttackerId,
      ...attack.attackerQueue,
    ].filter((id): id is number => id != null);
    if (building.buildingType === "base_camp") {
      ...
      this.startAttackReturn(
        ...
        survivingAttackers.map((id) => {
          const participant = attack.attackers.find(
            (p) => p.workerEntityId === id,
          );
          return {
            workerEntityId: id,
            buildingEntityId:
              participant?.sourceBuildingId ?? attack.targetEntityId,
            rank: this.getAttackParticipantRank(attack, id),
          };
        }),
      );
      ...
      this.startAttackReturn(
        attack,
        this.getCapturedMilitaryReturnAssignments(
          target,
          attack,
          survivingAttackers,
        ),
  ...
  private startAttackReturn(
    ...
    for (const assignment of attack.returnAssignments) {
      this.walkAttackSurvivorToBuilding(
        assignment.workerEntityId,
        assignment.buildingEntityId,
      );
    }
  ...
  private getCapturedMilitaryReturnAssignments(
    ...
    const building = target.getComponent(Building);
    const cap = Math.max(
      dataManager.getBuilding(building?.buildingType ?? "road")?.military
        ?.soldierCapacity ?? 0,
      0,
    );
    return survivingAttackers.map((id, index) => {
      ...
      return {
        ...
        buildingEntityId:
          index < cap
            ? target.id
            : (participant?.sourceBuildingId ?? target.id),
        rank: this.getAttackParticipantRank(attack, id),
      };
    });
  }

  private walkAttackSurvivorToBuilding(
    workerEntityId: number,
    buildingEntityId: number,
  ): void {
    const workerEntity = this.entities.find(
  ...
  private finishEnemyAttackReturn(attack: ActiveEnemyAttack): void {
    for (const assignment of attack.returnAssignments) {
      const workerEntity = this.entities.find(
        (e) => e.id === assignment.workerEntityId && e.active,
      );
      const buildingEntity = this.entities.find(
      ...
      if (building) {
        ...
        const cap = Math.max(
          def?.military?.soldierCapacity ??
            building.militaryGarrison?.length ??
            0,
          0,
        );
        if (
          cap > 0 &&
          (!building.militaryGarrison ||
            building.militaryGarrison.length !== cap)
        ) {
          building.militaryGarrison = Array.from({ length: cap }, () => null);
        }
        const slotIdx =
          building.militaryGarrison?.findIndex((slot) => slot === null) ?? -1;
    ...
    const target = this.entities.find(
      (e) => e.id === attack.targetEntityId && e.active,
    );
    if (
      target &&
      attack.attackerFactionId === PLAYER_FACTION &&
    ...
    if (
      ...
      this.markTerritoryDirty();
      this.refreshTerritoryIfDirty();
      this.applyFactionTerritoryAftermath(
        attack.attackerFactionId,
        PLAYER_FACTION,
      );
    }
  ...
  private getBuildingLootDropTile(
    entity: Entity,
  ): { x: number; y: number } | null {
    const pos = entity.getComponent(Position);
    const building = entity.getComponent(Building);
  ...
  private burnEnemyRealmAfterHqCapture(enemyFactionId: FactionId): void {
    const toBurn = this.entities.filter((entity) => {
      if (!entity.active || getEntityFaction(entity) !== enemyFactionId)
        return false;
      const building = entity.getComponent(Building);
      return (
        !!building &&
    ...
    for (const entity of toBurn)
      this.destroyConqueredEnemyCivilianBuilding(entity);
  ...
  private applyPlayerTerritoryPressureAftermath(): void {
    for (const factionId of ENEMY_FACTIONS) {
      if (
        this.hasCivilianBuildingInFactionTerritory(factionId, PLAYER_FACTION)
      ) {
  ...
  private hasCivilianBuildingInFactionTerritory(
    ...
    return this.entities.some((entity) => {
      if (!entity.active || getEntityFaction(entity) !== victimFactionId)
        return false;
      const building = entity.getComponent(Building);
      const pos = entity.getComponent(Position);
      if (
  ...
  private removeOrRetreatEnemyWorkersInPlayerLand(
    ...
    for (const entity of this.entities) {
      ...
      if (
        !pos ||
        !playerOwnedCells.has(`${Math.floor(pos.x)},${Math.floor(pos.y)}`)
      )
        continue;
  ...
  selectEntityAt(
    ...
    if (foundEntity) {
      ...
      if (
        !isPlayerOwned(foundEntity) &&
        !this.isEnemyAttackTarget(foundEntity)
      ) {
        this.selectedEntity = null;
        this.updateSelectionUI();
        return;
      ...
      const onPendingCell =
        pending !== null && pending.x === x && pending.y === y;
      const iconHit =
        onPendingCell &&
        !surveyMenuWasOpen &&
  ...
  dropEntity(x: number, y: number): void {
    ...
    if (pos && building) {
      ...
      if (
        ...
        console.warn(
          `❌ Cannot place building at (${x}, ${y}), reverting to original position`,
        );
    ...
    if (this.selectedEntity) {
      ...
      if (worker && this.getRoadSegmentForWorkerId(this.selectedEntity.id)) {
        eventBus.emit("road_worker:selected", {
          ...
          canReplaceWithDonkey: this.canReplaceRoadWorkerWithDonkey(
            this.selectedEntity.id,
          ),
          canReturnDonkeyToHq: this.canReturnRoadDonkeyToHq(
            this.selectedEntity.id,
          ),
        });
        return;
  ...
  getSaveData(): object {
    ...
    return {
      ...
      buildings: buildingsToSave.map((e) => {
        ...
        if (
          building?.militaryGarrison &&
          building.militaryGarrison.length > 0
        ) {
      ...
      pendingWellDemolitions: Array.from(
        this.pendingWellDemolitions.entries(),
      ).map(([entityId, demolishAt]) => ({
        entityId,
        demolishAt,
      })),
  ...
  loadSaveData(saveData: any): boolean {
    try {
      this.simulationNowMs =
        typeof saveData?.simulationNowMs === "number" &&
        Number.isFinite(saveData.simulationNowMs)
      ...
      this.demolitionSites = [];
      this.nextDemolitionSiteId = 1;
      this.enemyRealms = Array.isArray(saveData.enemyRealms)
        ? saveData.enemyRealms
        : [];
      this.activeEnemyAttacks = [];
      this.nextEnemyAttackId = 1;
      this.nextEnemyPressureCheckAt = 0;
      ...
      if (
        saveData.population &&
        typeof saveData.population.current === "number"
      ) {
        this.population.current = saveData.population.current;
      } else {
        this.population.current = dataManager.getStartingPopulation();
      }
      this.productionPriorities = this.normalizeProductionPriorities(
        saveData.productionPriorities,
      );
      this.buildingPriorities = this.normalizeBuildingPriorities(
        saveData.buildingPriorities,
      );
      ...
      for (const buildingData of saveData.buildings) {
        if (
          !buildingData.type ||
          buildingData.x === undefined ||
          buildingData.y === undefined
        )
          continue;
        ...
        if (building) {
          if (buildingData.state === "awaiting_materials") {
            ...
            building.constructionMaterials =
              buildingData.constructionMaterials || {};
          ...
          if (
            ...
            building.assignedToolSpecialist =
              buildingData.assignedToolSpecialist;
          }

          const capRestore = dataManager.getBuilding(
            buildingData.type as BuildingType,
          )?.military?.soldierCapacity;
          if (typeof capRestore === "number" && capRestore > 0) {
            ...
            if (
              Array.isArray(buildingData.militaryGarrison) &&
              building.militaryGarrison
            ) {
              for (let i = 0; i < building.militaryGarrison.length; i++) {
      ...
      if (saveData.roadSegments) {
        ...
        const donkeyRoadSegments = new Set<number>(
          Array.isArray(saveData.donkeyRoadSegments)
            ? saveData.donkeyRoadSegments
            : [],
        );
      ...
      if (Array.isArray(saveData.demolitionSites)) {
        const now = this.simulationNowMs;
        const maxAge =
          DEMOLITION_FIRE_DURATION_MS + DEMOLITION_SCORCH_DURATION_MS;
        ...
        const maxId = this.demolitionSites.reduce(
          (m, site) => Math.max(m, site.id),
          0,
        );
        const savedNextId =
          typeof saveData.nextDemolitionSiteId === "number"
            ? saveData.nextDemolitionSiteId
            : 1;
  ...
  private ensureMilitaryGarrisonWorkersPresent(): void {
    for (const ent of this.entities) {
      ...
      for (let i = 0; i < building.militaryGarrison.length; i++) {
        ...
        const workerEntity = createMilitaryWorker(
          pos.x + 0.5,
          pos.y + 0.5,
          slot.rank,
        );
