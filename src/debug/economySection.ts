...
function buildProductionMermaid(buildings: BuildingDefinition[]): string {
  ...
  outer: for (const b of buildings) {
    ...
    for (const group of p.inputsAny ?? []) {
      for (const resId of group.resourceTypes) {
        if (edgeCount >= maxEdges) {
          truncated = true;
          break outer;
        }
        const rId = rid(resId);
        nodes.set(rId, resourceLabel(resId));
        edges.push(`${rId} -->|"OR-slot ×${group.amount}"| ${bId}`);
        edgeCount++;
      }
    }
...
/** OR-input groups only (e.g. bread / ham / fish feeding mines). */
function buildInputsAnyMermaid(buildings: BuildingDefinition[]): string {
  const nodes = new Map<string, string>();
  const edges: string[] = [];
  for (const b of buildings) {
    const p = b.production;
    if (!p?.inputsAny?.length) continue;
    const bId = bid(b);
    nodes.set(bId, b.name);
    const time = p.productionTime;
    for (const group of p.inputsAny) {
      for (const resId of group.resourceTypes) {
        const rId = rid(resId);
        nodes.set(rId, resourceLabel(resId));
        edges.push(`${rId} -->|"×${group.amount}/cycle OR · ${time}s"| ${bId}`);
      }
    }
  }
  const lines: string[] = ['flowchart LR'];
  ...
  lines.push(...edges);
  if (edges.length === 0) {
    lines.push('empty((No inputsAny recipes — e.g. miner food not in data))');
  }
  return lines.join('\n');
}
...
export function economySection(): HTMLElement {
  ...
  wrap.appendChild(
    buildSectionToc('Jump within economy', [
      ...
      { id: 'economy-flow-miner-food', label: 'Chart: miner food' },
  ...
  wrap.appendChild(
    el(
      'p',
      'economy-intro',
      'From buildings.json via DataManager. Quantities are per placement (build) or per production cycle (recipe). Mines consume one unit per cycle from bread, ham, or fish (OR group — any one satisfies the slot). Gather buildings take inputs from the map (trees, rock, water) rather than stockpiles — those links are not drawn as resource→building edges below.'
    )
  );
  ...
  for (const b of buildings) {
    ...
    if (p) {
      ...
      for (const group of p.inputsAny ?? []) {
        for (const resId of group.resourceTypes) {
          const info = rmap.get(resId) ?? { produced: [], recipeIn: [], build: [], tool: [] };
          info.recipeIn.push(`${b.name} ×${group.amount}/cycle (OR-input)`);
          rmap.set(resId, info);
        }
      }
    }
  ...
  const hMine = el('h3', '', 'Flow: miner food → mines (OR-input per cycle)');
  hMine.id = 'economy-flow-miner-food';
  wrap.appendChild(hMine);
  wrap.appendChild(
    el(
      'p',
      'economy-intro',
      'Each mine needs one of bread, ham, or fish each production cycle (same amount from any listed resource). Parallel edges mean alternative foods, not cumulative.'
    )
  );
  const mwMine = el('div', 'mermaid-wrap');
  const preMine = el('pre', 'mermaid');
  preMine.textContent = buildInputsAnyMermaid(buildings);
  mwMine.appendChild(preMine);
  wrap.appendChild(mwMine);
