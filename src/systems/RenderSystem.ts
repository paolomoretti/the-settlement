...
export class RenderSystem extends System {
  ...
  update(_deltaTime: number): void {
    ...
    const visibleEntities = this.entities.filter(entity => {
      ...
      return (
        pos.x + width >= viewportBounds.minX &&
  ...
  private getConstructionSmokeOrigin(building: Building): {
    x: number;
    y: number;
  } {
  ...
  private getDemolitionFireOrigin(site: DemolitionSiteVisual): {
    x: number;
    y: number;
  } {
  ...
  private getDemolitionFireSize(site: DemolitionSiteVisual): {
    width: number;
    height: number;
    density: number;
  } {
  ...
  private renderDisconnectedBuildingRoadHints(
    gridX: number,
    gridY: number,
    width: number,
    depth: number
  ...
  private renderDemolitionScorches(viewportBounds: {
    ...
    for (const site of this.demolitionSites) {
      ...
      const fade =
        ageMs < DEMOLITION_FIRE_DURATION_MS
          ? 1
  ...
  private renderRoads(viewportBounds: {
    minX: number;
    maxX: number;
    minY: number;
    maxY: number;
  ...
  private getRoadWalkScreenPosition(movable: Movable): { x: number; y: number } | null {
    ...
    const segmentStart =
      movable.progress === 0 && movable.currentPathIndex > 0
    ...
    if (linearStart || linearTarget) {
      const a = linearStart ?? {
        x: startCenter.x + startHub.x,
        y: startCenter.y + startHub.y,
      };
      const b = linearTarget ?? {
        x: targetCenter.x + targetHub.x,
        y: targetCenter.y + targetHub.y,
      };
      return {
        x: a.x + (b.x - a.x) * p,
        y: a.y + (b.y - a.y) * p,
    ...
    const first = {
      ...
      c: {
        x: startCenter.x + startHub.x * 1.25,
        y: startCenter.y + startHub.y * 1.25,
      },
    ...
    const second = {
      ...
      c: {
        x: targetCenter.x + targetHub.x * 1.25,
        y: targetCenter.y + targetHub.y * 1.25,
      },
  ...
  private getWildRabbitGridPosition(rabbit: WildRabbit): {
    gx: number;
    gy: number;
  } {
  ...
  private renderDepthSorted(
    ...
    const { blocks: mountainBlocks, covered: mountainCovered } =
      this.findMountainBlocks(viewportBounds);
  ...
  private renderJunctionItems(viewportBounds: {
    minX: number;
    maxX: number;
    minY: number;
    maxY: number;
